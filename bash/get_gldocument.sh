#!/bin/bash
# name:    get_gldocument.sh
# process: Retrieve a list of 3 gl documents
#          GET /glrestapi/rest/v1/gldocument?limit=3
# Current status is:
...
curl -u "${CMIC_USERNAME}:${CMIC_PASSWORD}" \
  --url "${CMIC_BASE_URL}/glrestapi/rest/v1/gldocument?limit=3" \
